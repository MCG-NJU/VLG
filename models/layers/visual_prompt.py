...
def trunc_normal_actionclip(x, mean=0., std=1.):
...
class TAggregate(nn.Module):
    def __init__(self, clip_length=None, embed_dim=2048, n_layers=6):
        ...
        with torch.no_grad():
            trunc_normal_actionclip(self.pos_embed, std=.02)
            trunc_normal_actionclip(self.cls_token, std=.02)
        self.apply(self._init_weights)

    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            with torch.no_grad():
                trunc_normal_actionclip(m.weight, std=.02)
...
class VisualPrompt(nn.Module):
    def __init__(self, sim_head, embed_dim, context_length, transformer_width, T=8, num_layers=6):
        super().__init__()
        ...
        if self.sim_header in ["LSTM", "Transf", "Transf_cls", "Conv_1D"]:
        ...
        if self.sim_header == "Transf" :
            self.transformer = TemporalTransformer(width=embed_dim, layers=self.num_layers, heads=transformer_heads)
            print(f'layer={self.num_layers}')
