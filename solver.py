...
def _optimizer(config, model):
    ...
    if config.opt == 'adam':
        fusion_model_params = list(map(id, fusion_model.parameters()))
        clip_params = filter(lambda p: id(p) not in fusion_model_params,
                             model.parameters())
        optimizer = optim.Adam([{'params': clip_params},
         {'params': fusion_model.parameters(), 'lr': config.lr * config.f_ratio}],
                               lr=config.lr, betas=(0.9, 0.98), eps=1e-8,
        ...
        clip_params = filter(lambda p: id(p) not in fusion_model_params,
                             model.parameters())
        optimizer = optim.SGD([{'params': clip_params},
         {'params': fusion_model.parameters(), 'lr': config.lr * config.f_ratio}],
                              config.lr,
        ...
        fusion_model_params = list(map(id, fusion_model.parameters()))
        text_params = filter(lambda p: id(p) not in vision_params and id(p) not in fusion_model_params,
                             model.parameters())

        optimizer = optim.AdamW([{'params': text_params},
                                 {'params': model.visual.parameters(), 'lr': config.lr * config.ratio},
                                 {'params': fusion_model.parameters(), 'lr': config.lr * config.f_ratio}],
                                betas=(0.9, 0.98), lr=config.lr, eps=1e-8,
                                weight_decay=config.weight_decay)  # Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
        for param_group in optimizer.param_groups:
            print(param_group['lr'])
        print('AdamW')
    else:
        raise ValueError('Unknown optimizer: {}'.format(config.opt))
    return optimizer
...
def _lr_scheduler(config,optimizer):
    if config.sched == 'cosine':
        ...
        if isinstance(config.decay_epochs, list):
            milestones = config.decay_epochs
        elif isinstance(config.decay_epochs, (int, float)):
            milestones = [
                config.decay_epochs * (i + 1)
                for i in range(config.epochs //
                               config.decay_epochs)]
        else:
            raise ValueError("error learning rate decay step: {}".format(type(config.decay_epochs)))
        ...
        raise ValueError('Unknown lr scheduler: {}'.format(config.sched))
    return lr_scheduler
